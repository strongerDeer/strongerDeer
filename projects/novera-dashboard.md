...
metrics:
  [
    ...
    "실사용자 지표 통과: LCP 1.44초·INP 88ms·CLS 0",
    "GA4·Search Console·어드민 데이터를 반복 관측 가능한 기준으로 통합",
  ]
url: ""
...
- **사용자**: 운영 담당자·경영진
- **목적**: 이벤트 목표 도달 여부, 유입·참여·가입 흐름, 채널별 성과를 빠르게 확인
- **핵심 기준**: PV, 순방문, 일평균 DAU, 회원가입, 목표 도달률

## 화면 구성

- 상단 기간·회차·채널 필터와 핵심 KPI 카드
- 이벤트 오픈 후 7/14/30일 상대기간 비교
- 온라인·오프라인·외부 채널 유입과 전환 구성
- GA4·Search Console·Admin 데이터의 식별률·커버리지 표시
- 카페24 외부 채널을 포함한 통합 성과 뷰
...
- **사용자**: 프론트엔드·디자인
- **목적**: 배포 후 성능·접근성·SEO·이미지·번들 회귀 감지
- **판정 기준**: 실사용자 지표를 합격 기준으로 사용하고 Lighthouse는 주간 변화 비교에 활용

## 화면 구성

- 페이지별 Core Web Vitals와 Lighthouse 점수
- 이미지 전송량·번들 크기·타입·린트 오류
- 운영 release 커밋 기준의 이전 배포 대비 변화
- 회귀·측정 노이즈 구분과 주간 분석 코멘트
- canonical·hreflang·JSON-LD·sitemap·robots 자동 점검

![Shop 품질 모니터링 — 재측정·회귀 판정과 주간 자동 분석 코멘트](/strongerDeer/project/nad_quality.jpg)

동일 배포본 재측정 여부, 이미지·번들·성능 변화, 회귀·노이즈, 주간 분석 코멘트 구성.

![실사용자 지표(CrUX) — LCP 1.44s·INP 88ms·CLS 0·LCP 구성 분해와 28일 추세](/strongerDeer/project/nad_crux.jpg)

LCP 1.44s·INP 88ms·CLS 0 판정과 LCP 구성 요소, 28일 추세 구성.
