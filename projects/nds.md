...
metrics:
  [
    "배포한 공통 모듈 대부분을 자사 쇼핑몰에서 실사용 — 로그인·주문·상품·이벤트 등 코드 166개 파일 적용",
...
highlights:
  [
    ...
    { value: "대부분 실사용", label: "배포한 공통 모듈 대부분을 자사 쇼핑몰 화면에서 실제 사용" },
...
- **첫 운영 제품에 UI 기준 적용** — 자사 쇼핑몰 전역을 NDS `ThemeProvider`로 감싸 토큰·테마를 전역 적용. 배포한 공통 모듈 대부분을 실사용하고, 로그인·주문·상품·이벤트 등 코드 166개 파일에서 NDS를 직접 소비. 최소 44px 터치 영역·아이콘 버튼 접근성 이름·다중 모달 포커스를 공통화
...
**한계와 다음 과제** — 첫 운영 소비 제품 적용까지 진행했지만, 담당 종료 시점까지 후속 제품 확장·장기 마이그레이션 지표를 충분히 확인하지 못함. 공통 컴포넌트의 수보다 제품에서 반복되는 문제를 어디까지 라이브러리의 기본값·제약으로 가져올지 결정하는 일이 더 중요했고, 디자인 시스템을 만드는 것과 조직에 채택시키는 것은 다른 문제라는 점을 배움. 다음에는 도입 우선순위, 하위 호환이 깨지는 변경·마이그레이션 정책, 사용 현황 측정을 초기 설계에 함께 포함하고자 함.
